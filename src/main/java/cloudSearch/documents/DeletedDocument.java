...
public class DeletedDocument {
    ...
    /**
     * A document version number for the add or delete operation.
     * The version is used to guarantee that older updates aren't accidentally applied, and to provide control over the ordering of concurrent updates to the service.
     * The document service guarantees that the update with the highest version will be applied and remain there until an add or delete operation with a higher version number and the same document ID is received.
