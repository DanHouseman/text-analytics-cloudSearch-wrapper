...
/**
 * Official documentation from Amazon: http://docs.aws.amazon.com/cloudsearch/latest/developerguide/search-api.html
 */
...
public class Query {
     ...
     * Use the size parameter to control the number of hits you want to include in each response.
     * You can specify either the cursor or start parameter in a request, they are mutually exclusive.
     * For more information, see Paginating Results.
     *
     * To get the first cursor, specify cursor=initial in your initial request.
     * In subsequent documents, specify the cursor value returned in the hits section of the response.
     *
     ...
     *
     * search?q=monkey&cursor=initial&size=100&return=_no_fields
     * {
     *    "status": {
     *       "rid": "+/TuXeD0P4NTSlol=",
     *       "time-ms": 15
     ...
     *       "found": 314,
     *       "start": 0,
     *       cursor": "CurSorDanVegaMONKEYTHISisACURS0RTh1nGYuPHA",
     *       "hit": [
     *          {"id": "tt0123456"},
     *          {"id": "tt0123457"},
     *          ...
     ...
     *
     * search?q=monkey&cursor=CurSorDanVegaMONKEYTHISisACURS0RTh1nGYuPHA&size=100
     *
     * Required: No
     */
    ...
    /**
     * Defines an expression that can be used to sort results or specify search or filter criteria.
     * You can also specify an expression as a return field. For more information about defining and using expressions.
     * See Configuring Expressions for more information.
     *
     * You can define and use multiple expressions in a search request.
     * For example, the following request creates two expressions that are used to sort the results and includes them in the search results:
     *
     * search?q=(and (term field=keywords 'monkey')(term field=questionlabel 'OE_Future Plans'))&q.parser=structured
     * &expr.expression1=sentiment*latentScore // for example
     * &expr.expression2=(1/_score)*sentiment // for example
     * &sort=expression1 desc,expression2 desc
     * &return=_score,questionlabel,sentiment,respondentkey,latentscore,freeformresptext,daydate,expression1,expression2
     *
     * Required: No
     */
     ...
     * You use fq in conjunction with the q parameter to filter the documents that match the constraints specified in the q parameter.
     * Specifying a filter just controls which matching documents are included in the results, it has no effect on how they are scored and sorted.
     * The fq parameter supports the full structured query syntax. For more information about using filters, see Filtering Matching Documents.
     ...
     * When your search index is partitioned across multiple search instances, by default Amazon CloudSearch only returns results if every partition can be queried.
     * This means that the failure of a single search instance can result in 5xx (internal server) errors.
     *
     * When you specify partial=true. Amazon CloudSearch returns whatever results are available and includes the percentage of documents searched in the search results (percent-searched).
     * This enables us to gracefully degrade the search experience.
     * For example, rather than displaying no results, you could display the partial results and a message indicating that the results might be incomplete due to a temporary system outage.
     *
     * Default: False
     *
     * Required: No
     ...
     * How you specify the search criteria depends on the query parser used for the request and the parser options specified in the q.options parameter.
     * By default, the simple query parser is used to process documents.
     * To use the structured, lucene, or dismax query parser, you must also specify the q.parser parameter.
     * For more information about specifying search criteria, see Searching Your Data with Amazon CloudSearch.
     *
     * Required: Yes
     */
    public String query;
     ...
     * The options are specified as a JSON object, for example: q.options={defaultOperator: 'or', fields: ['freeformTextResp^monkey','keywords']}.
     *
     * The options you can configure vary according to which parser you use:
     * defaultOperator—The default operator used to combine individual terms in the search string.
     * For example: defaultOperator: 'or'. For the dismax parser, you specify a percentage that represents the percentage of terms in the search string (rounded down) that must match, rather than a default operator.
     * A value of 0% is the equivalent to OR, and a value of 100% is equivalent to AND.
     * The percentage must be specified as a value in the range 0-100 followed by the percent (%) symbol.
     * For example, defaultOperator: 50%. Valid values: and, or, a percentage in the range 0%-100% (dismax).
     * Default: and (simple, structured, lucene) or 100 (dismax). Valid for: simple, structured, lucene, and dismax.
     *
     * fields—An array of the fields to search when no fields are specified in a search.
     * If no fields are specified in a search and this option is not specified, all text and text-array fields are searched.
     * You can specify a weight for each field to control the relative importance of each field when Amazon CloudSearch calculates relevance scores.
     * To specify a field weight, append a caret (^) symbol and the weight to the field name.
     * For example, to boost the importance of the entities field over the keywords field you could specify: fields: ['keywords^5','entities'].
     * Valid values: The name of any configured field and an optional numeric value greater than zero.
     * Default: All text and text-array fields. Valid for: simple, structured, lucene, and dismax.
     *
     * operators—An array of the operators or special characters you want to disable for the simple query parser.
     * If you disable the and, or, or not operators, the corresponding operators (+, |, -) have no special meaning and are dropped from the search string.
     * Similarly, disabling prefix disables the wildcard operator (*) and disabling phrase disables the ability to search for phrases by enclosing phrases in double quotes.
     * Disabling precedence disables the ability to control order of precedence using parentheses.
     * Disabling near disables the ability to use the ~ operator to perform a sloppy phrase search.
     * Disabling the fuzzy operator disables the ability to use the ~ operator to perform a fuzzy search.
     * Escape disables the ability to use a backslash (\) to escape special characters within the search string.
     * Disabling whitespace is an advanced option that prevents the parser from tokenizing on whitespace, which can be useful for Vietnamese. (It prevents Vietnamese words from being split incorrectly.)
     * For example, you could disable all operators other than the phrase operator to support just simple term and phrase queries: operators:['and', 'not', 'or', 'prefix'].
     * Valid values: and, escape, fuzzy, near, not, or, phrase, precedence, prefix, whitespace.
     ...
     * You can specify a weight for each field to boost that score.
     * The phraseSlop option controls how much the matches can deviate from the search string and still be boosted.
     * To specify a field weight, append a caret (^) symbol and the weight to the field name.
     * For example, to boost phrase matches in the freeformTextResp field over the entities field, you could specify: phraseFields:['entities^3', 'freeformTextResp']
     * Valid values: The name of any text or text-array field and an optional numeric value greater than zero.
     * Default: No fields. If you don't specify any fields with phraseFields, proximity scoring is disabled even if phraseSlop is specified.
     * Valid for: dismax.
     *
     * phraseSlop—An integer value that specifies how much matches can deviate from the search phrase and still be boosted according to the weights specified in the phraseFields option.
     ...
     * Valid for: dismax.
     *
     * explicitPhraseSlop—An integer value that specifies how much a match can deviate from the search phrase when the phrase is enclosed in double quotes in the search string.
     * (Phrases that exceed this proximity distance are not considered a match.)
     * explicitPhraseSlop: 5.
     * Valid values: positive integers.
     * Default: 0.
     * Valid for: dismax.
     *
     * tieBreaker—When a term in the search string is found in a document's field, a score is calculated for that field based on how common the word is in that field compared to other documents.
     * If the term occurs in multiple fields within a document, by default only the highest scoring field contributes to the document's overall score.
     * You can specify a tieBreaker value to enable the matches in lower-scoring fields to contribute to the document's score.
     * That way, if two documents have the same max field score for a particular term, the score for the document that has matches in more fields will be higher.
     ...
     * For example, the following query searches for the term monkey in the keywords, entities, and concepts fields and sets tieBreaker to 0.1:
     *
     * q=monkey&q.parser=dismax&q.options={fields:['keywords', 'entities', 'concepts'], tieBreaker: 0.1}
     * If monkey occurs in all three fields of a document and the scores for each field are keywords=1, entities=3, and concepts=1, the overall score for the term monkey is:
     *
     * 3 +  0.1 * (1+1) = 3.2
    ...
    /**
     * Specifies which query parser to use to process the request: simple, structured, lucene, and dismax.
     * If q.parser is not specified, Amazon CloudSearch uses the simple query parser.
     *
     * simple—perform simple searches of text and text-array fields.
     * By default, the simple query parser searches all text and text-array fields.
     * You can specify which fields to search by with the q.options parameter.
     ...
     * For more information, see Searching Your Data with Amazon CloudSearch.
     *
     * structured—perform advanced searches by combining multiple expressions to define the search criteria.
     * You can also search within particular fields, search for values and ranges of values, and use advanced options such as term boosting, matchall, and near.
     ...
     *
     * dismax—search using the simplified subset of the Apache Lucene query parser syntax defined by the DisMax query parser.
    ...
    /**
     * The field and expression values to include in the response, specified as a comma-separated list.
     * By default, a search response includes all return enabled fields (return=_all_fields).
     * To return only the document IDs for the matching documents, specify return=_no_fields.
     * To retrieve the relevance score calculated for each document, specify return=_score.
     * You specify multiple return fields as a comma separated list.
     * For example, return=freeFormTextResp, _score returns just the freeFormTextResp and relevance score of each matching document.
     *
     * Type: String
     *
     * Required: No
     ...
     * You must specify the sort direction (asc or desc) for each field.
     * For example, sort=year desc,title asc.
     * You can specify a maximum of 10 fields and expressions.
     * To use a field to sort results, it must be sort enabled in the domain configuration.
     * Array type fields cannot be used for sorting.
     * If no sort parameter is specified, results are sorted by their default relevance scores in descending order: sort=_score desc.
    ...
    /**
     * The offset of the first search hit you want to return.
     * You can specify either the start or cursor parameter in a request, they are mutually exclusive.
     * For more information, see Paginating Results.
     *
